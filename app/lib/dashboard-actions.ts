...
export async function resolveSignalOnchain(
  ...
  if (!signalBondAddress || !signal.onchainId) {
    throw new Error("PitchSide Signals address or onchain signal id is not configured.");
  }
...
export async function ensureXLayerNetwork(): Promise<Hex> {
  if (!window.ethereum) {
    throw new Error("No injected wallet found.");
  }

  const chainId = numberToHexChainId(xLayerChain.id);
  ...
  try {
    ...
    await window.ethereum.request({
      ...
      params: [
        {
          chainId,
          chainName: xLayerChain.name,
          nativeCurrency: xLayerChain.nativeCurrency,
          rpcUrls: xLayerChain.rpcUrls.default.http,
          blockExplorerUrls: xLayerChain.blockExplorers
            ? [xLayerChain.blockExplorers.default.url]
            : undefined,
        },
      ],
    });
...
function sanitizeErrorMessage(message: string): string {
  return message
    ...
    .replace(/https:\/\/xlayertestrpc\.okx\.com\/terigon/g, "[X Layer RPC endpoint]")
    .replace(/https:\/\/testrpc\.xlayer\.tech\/terigon/g, "[X Layer RPC endpoint]")
    .replace(/swrm_[A-Za-z0-9_-]+/g, "[RPC token]");
}

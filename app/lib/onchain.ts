...
export async function waitForOnchainTx(hash: Hex) {
  ...
  if (receipt.status === "reverted") {
    throw new Error("Transaction reverted on X Layer. Check the wallet activity for details.");
  }
...
function contractSignalToSignal(
  ...
  return {
    ...
    reasoning:
      "This signal is loaded from PitchSide Signals contract storage. Source data and thesis hashes are anchored on X Layer for auditability.",
    sources: ["pitchside-contract", "xlayer-event-log"],
  };
}

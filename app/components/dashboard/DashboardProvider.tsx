...
export default function DashboardProvider({
  ...
  const xLayerChainHex = useMemo(() => numberToHexChainId(xLayerChain.id), []);
  const walletOnXLayer = walletChainId ? sameChainId(walletChainId, xLayerChainHex) : false;
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      try {
        const chainId = await ensureXLayerNetwork();
        setWalletChainId(chainId);
      } catch (error) {
        setWalletError(normalizeError(error));
      }
  ...
  const publishProposal = useCallback(async (overrides?: { stakeUsdc?: number }) => {
    ...
    if (contractsConfigured) {
      if (!walletAddress) {
        setWalletError("Connect wallet before publishing this signal to X Layer.");
        return;
      }
